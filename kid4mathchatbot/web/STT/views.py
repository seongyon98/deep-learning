...
def find_similar_question(question):
    ...
    similarities = cosine_similarity(input_question_vector, cached_question_vectors).flatten()
...
def qa_process(request):
    ...
    if request.method == "POST":
        ...
        current_time = timezone.now().astimezone(kst).strftime(
            "%Y-%m-%d %H:%M:%S")

        if results and isinstance(results[0], Lecture):
            show_cards = True
            answer_str = ", ".join(
                [result.lecture_title for result in results])
        else:
