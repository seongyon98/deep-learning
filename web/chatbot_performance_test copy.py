...
async def write_to_csv(file_path, data):
    ...
    with open(file_path, mode="w", newline="", encoding="utf-8") as file:
        fieldnames = ["Question", "Expected Keyword", "Predicted Title", "Similarity"]
...
async def evaluate_chatbot_performance(test_data, output_csv_path):
    ...
    for data in test_data:
        ...
        predicted_lectures = await chatbot_response(question)  # 수정된 부분
        ...
        # 정확도 계산을 위한 함수 호출
        is_correct = calculate_accuracy(expected_keywords, predicted, complex_question)
        ...
        for expected_keyword in expected_keywords:
            for predicted_title in predicted:
                similarity = difflib.SequenceMatcher(
                    None, expected_keyword, predicted_title
                ).ratio()
                print(
    ...
    average_response_time = (
        total_response_time / total_questions if total_questions > 0 else 0
    )
